...
class FileRenamerApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Batch Renamer v1.9 - Developed by Nihad Square")
        self.original_names = []
        self.renamed_names = []
        self.undo_data = None
    ...
    def open_about(self):
        webbrowser.open("https://github.com/NihadSquare/BatchRenamer")

    def open_tutorial(self):
        webbrowser.open("https://www.youtube.com/watch?v=your_tutorial_id")

    def open_contact(self):
        webbrowser.open("https://nihad.carrd.co")
    ...
    def build_gui(self):
        ...
        try:
            # Load the PNG icon
            self.app_icon = tk.PhotoImage(file="BatchRenamer.png")
            heading = ttk.Label(
                self.root,
                image=self.app_icon,
                compound="center",)
        ...
        reset_button = tk.Button(self.root, text="↺", command=self.reset_all_fields,
            ...
            padx=2,
            pady=1)
        reset_button.place(relx=0.935, y=10, anchor="ne")
        Hovertip(reset_button, "Reset form")


        # Section 1: Folder Selection
        frame1 = ttk.LabelFrame(self.root, text="◼ Folder Selection")
        ...
        frame2 = ttk.LabelFrame(self.root, text="◻ Character Cleanup")
        frame2.pack(fill="x", padx=10, pady=5)
        ttk.Label(frame2, text="Characters to Remove:").grid(row=0, column=0, padx=(20, 5))
        ttk.Entry(frame2, textvariable=self.remove_chars).grid(row=0, column=1, padx=(0, 20))
        ttk.Label(frame2, text="Replace With:").grid(row=0, column=2, padx=5)
        ttk.Entry(frame2, textvariable=self.replace_with).grid(row=0, column=3, padx=(0, 20))
        ...
        frame2b = ttk.LabelFrame(self.root, text="◻ Word-Based Cleanup")
        frame2b.pack(fill="x", padx=10, pady=5)
        ttk.Label(frame2b, text="Words to Remove:").grid(row=0, column=0, padx=(20, 5))
        ttk.Entry(frame2b, textvariable=self.word_remove).grid(row=0, column=1, padx=(0, 20))
        ttk.Label(frame2b, text="Replace With:").grid(row=0, column=2, padx=5)
        ttk.Entry(frame2b, textvariable=self.word_replace).grid(row=0, column=3, padx=(0, 20))
        ...
        frame3 = ttk.LabelFrame(self.root, text="◻ New Naming Options")
        frame3.pack(fill="x", padx=10, pady=5)
        ttk.Label(frame3, text="Common Name:").grid(row=0, column=0, padx=(20, 5))
        ttk.Entry(frame3, textvariable=self.common_name).grid(row=0, column=1, padx=(0, 20))
        ...
        ttk.Entry(frame3, textvariable=self.start_number, width=5, justify='center').grid(row=0, column=4, padx=(0, 20))
        ttk.Label(frame3, text="Position:").grid(row=0, column=5, padx=5)
        ttk.Combobox(frame3, textvariable=self.serial_position, values=["Before", "After"], width=7, state="readonly").grid(row=0, column=6, padx=(0, 20))

        # Section 4: Filters and Case
        frame4 = ttk.LabelFrame(self.root, text="◻ File Type and Filters")
        frame4.pack(fill="x", padx=10, pady=5)
        ttk.Label(frame4, text="File Types:").pack(side="left", padx=(20, 5))
        ttk.Combobox(frame4, textvariable=self.file_type_filter, values=["All Files", ".png", ".jpg", ".jpeg", ".webp", ".pdf", ".docx", ".svg", ".ai", ".psd", ".txt", ".csv"], width=10, state="readonly").pack(side="left", padx=(0, 20))
        ttk.Label(frame4, text="Case:").pack(side="left", padx=5)
        ttk.Combobox(frame4, textvariable=self.case_option, values=["No Change", "lowercase", "UPPERCASE", "Title Case"], width=15, state="readonly").pack(side="left", padx=(0, 20))
        ttk.Checkbutton(frame4, text="Skip Hidden/System Files", variable=self.skip_hidden).pack(side="left", padx=40)

        # Section 5: Preview and Rename
        frame5 = ttk.LabelFrame(self.root)
        ...
        self.tree.pack(fill="both", expand=True, padx=5, pady=0)
        ...
        self.rename_count_label.pack(fill="x", padx=20, pady=(0, 5))
        self.status_label = ttk.Label(self.rename_count_label, text="")
        ...
        ttk.Label(self.root, text="Developed by Nihad Square  |  Batch Renamer v1.9",
...
if __name__ == '__main__':
    ...
    root.minsize(690, 765)  # Minimum usable size
    root.maxsize(1000, 900)  # Maximum comfortable size
    
    # Auto-adjust for small screens
    screen_width = root.winfo_screenwidth()
    screen_height = root.winfo_screenheight()
    if screen_height < 800:
        root.geometry(f"800x{screen_height-100}")  # Leave 100px for taskbar
